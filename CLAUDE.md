...
- **Drafts & Quick add** (`Piece.draft`, `views_curate.QuickAddView`): `/curate/pieces/quick-add/` is a phone-first bulk capture page — each uploaded photo becomes a piece with no title and the shared placeholder artist/location rows. Whether captures start as drafts is the curator-editable `SiteSettings.quick_add_drafts` (default True; off = they publish immediately). Photos run through the same `validate_image_upload` as the normal form, and each is created inside its own savepoint so one storage failure can't roll back the whole batch (ATOMIC_REQUESTS wraps the POST).
  ...
  - **URL stability is a STORED latch, `Piece.slug_settled`** — never re-derived from current state, or re-drafting a published piece silently re-opens its permanent URL. A piece **commits its URL as soon as it has a title**; before that it carries a throwaway slug derived from its own id (`quick-add-<hex>`), unique by construction, so captures never compete for a shared namespace and a completed capture's URL is never reissued to another piece. The settled slug is `<artist>-<title>`, with the **placeholder artist omitted** so a title-only pass reads `/morning-tide/` rather than `/not-set-morning-tide/`. Being NFC-tagged also settles the URL — checked BEFORE the settle branch, so a tag on the wall keeps the exact address it encodes rather than being re-minted to a tidier one. The tag-write path (link and `?from=tag` handler) refuses while a URL is provisional AND while a piece is a draft, since a draft's page 404s for everyone else. Known gap: settling is permanent, so a typo typed into a title is baked into the URL — editable slugs with redirects from retired ones are the intended remedy. `title` is `blank=True` so a partial pass can be saved; use `Piece.display_title` for human-facing strings. Guarded by `SlugStabilityTests`/`ThrowawayUrlLifetimeTests` in `test_quick_add.py`.
